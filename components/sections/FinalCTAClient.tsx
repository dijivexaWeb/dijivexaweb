...
export function FinalCTAClient({ locale, content }: { locale: string; content: FinalCTAContent }) {
    ...
    <section ref={ref} className="py-24 relative overflow-hidden"
      ...
      <div className="relative max-w-3xl mx-auto px-4 sm:px-6 text-center">
        <motion.div initial={{ opacity: 0, y: 32 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.8 }}>
          <h2 className="text-4xl sm:text-5xl font-bold text-white mb-5 leading-tight">{content.heading}</h2>
          <p className="text-lg mb-10 max-w-xl mx-auto" style={{ color: "#94A3B8" }}>{content.subheading}</p>
          ...
          <p className="mt-6 text-sm" style={{ color: "#475569" }}>{content.footnote}</p>
        </motion.div>
      </div>
    </section>
  );
