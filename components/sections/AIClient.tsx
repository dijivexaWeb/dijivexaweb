...
interface AIContent {
  ...
  features: string[]; disclaimer: string; cta_text: string; cta_href: string;
}

export function AIClient({ locale, content }: { locale: string; content: AIContent }) {
    ...
    <section ref={ref} className="py-24 relative overflow-hidden"
      ...
      <div className="relative max-w-7xl mx-auto px-4 sm:px-6">
        <div className="grid lg:grid-cols-2 gap-12 items-center">
          <motion.div initial={{ opacity: 0, scale: 0.85, x: -40 }} whileInView={{ opacity: 1, scale: 1, x: 0 }}
            viewport={{ once: true }} transition={{ duration: 0.9, ease: [0.22, 1, 0.36, 1] }} className="flex justify-center">
            <div className="relative w-72 h-72">
              <motion.div animate={{ rotate: 360 }} transition={{ duration: 20, repeat: Infinity, ease: "linear" }}
                className="absolute inset-0 rounded-full border-2 border-dashed" style={{ borderColor: "rgba(59,130,246,0.2)" }} />
              <motion.div animate={{ rotate: -360 }} transition={{ duration: 14, repeat: Infinity, ease: "linear" }}
                className="absolute inset-8 rounded-full border" style={{ borderColor: "rgba(99,102,241,0.25)" }} />
              <div className="absolute inset-0 flex items-center justify-center">
                <motion.div animate={{ scale: [1, 1.1, 1], opacity: [0.6, 1, 0.6] }} transition={{ duration: 3, repeat: Infinity, ease: "easeInOut" }}
                  ...
                  <div className="w-24 h-24 rounded-2xl flex items-center justify-center text-5xl"
                    style={{ background: "linear-gradient(135deg, rgba(37,99,235,0.3), rgba(99,102,241,0.3))", border: "1px solid rgba(99,102,241,0.4)" }}>🤖</div>
                </motion.div>
              </div>
              {[0, 60, 120, 180, 240, 300].map((deg) => (
                <motion.div key={deg} animate={{ rotate: 360 }} transition={{ duration: 8, repeat: Infinity, ease: "linear", delay: deg / 360 * 8 }}
                  className="absolute inset-0" style={{ transformOrigin: "50% 50%" }}>
                  <div className="absolute w-2.5 h-2.5 rounded-full"
                    style={{ background: `hsl(${deg + 220}, 80%, 65%)`, top: "calc(50% - 136px)", left: "calc(50% - 5px)", transform: `rotate(${deg}deg) translateY(136px)`, boxShadow: `0 0 8px hsl(${deg + 220}, 80%, 65%)` }} />
                </motion.div>
              ))}
              <motion.div animate={{ y: [0, -8, 0] }} transition={{ duration: 3, repeat: Infinity }}
                className="absolute -right-4 top-8 px-3 py-1.5 rounded-full text-xs font-semibold border"
                style={{ background: "rgba(13,148,136,0.15)", borderColor: "rgba(45,212,191,0.3)", color: "#2DD4BF", backdropFilter: "blur(10px)" }}>✓ Analiz Hazır</motion.div>
              <motion.div animate={{ y: [0, 8, 0] }} transition={{ duration: 4, repeat: Infinity, delay: 1 }}
                ...
                style={{ background: "rgba(99,102,241,0.15)", borderColor: "rgba(129,140,248,0.3)", color: "#A5B4FC", backdropFilter: "blur(10px)" }}>🧬 Gemini Pro</motion.div>
            </div>
          </motion.div>
          ...
          <div className="gsap-reveal-right">
            ...
            <p className="leading-relaxed mb-8" style={{ color: "#94A3B8" }}>{content.subheading}</p>
              ...
              {content.features.map((f) => (
                <li key={f} className="flex items-center gap-3 text-sm" style={{ color: "#CBD5E1" }}>
                  <span className="w-5 h-5 rounded-full flex items-center justify-center shrink-0 text-xs"
                    style={{ background: "rgba(45,212,191,0.15)", color: "#2DD4BF" }}>✓</span>{f}
                </li>
              ))}
            </ul>
            <div className="flex items-start gap-2.5 p-4 rounded-xl mb-6"
              ...
              <p className="text-xs leading-relaxed" style={{ color: "#94A3B8" }}>{content.disclaimer}</p>
            </div>
